...
export const sendMessage = async (message, delivery) => {
  await setDoc(doc(db, "messages", `${delivery.client.clientID}-(${delivery.contact.number})-(${new Date().toLocaleString("sv", { timeZone: "Europe/Berlin" })})`), message)
  await setDoc(doc(db, "clients", `${delivery.client.clientID}/messages/(${delivery.contact.number})-(${new Date().toLocaleString("sv", { timeZone: "Europe/Berlin" })})`), message)
}

export function MessageAlert({ handleVisibility, visible, inProgressDelivery, delivery }) {
  const { dropDownRiders } = UserAuth();
  ...
  const handleSubmit = async (e, type) => {
    e.preventDefault();
    if (checkEmptyValues(inputFields[0].rider) && inputFields[0].assignedWarehouse.length !== 0) {
      if (type === "SMS") {
        delivery.SMS.headsup = true
      ...
      inProgressDelivery(inputFields[0]);
  ...
  const setRiderField = (event) => {
    ...
    if(event){
      ...
      Dein OneSpot Team. Let’s go! 🚴
      `);      
      auxInputFields.rider = { name: event.value, lastName: event.lastName, number: event.number }
    }
...
const inProgressDelivery = async (delivery) => {
  delivery.fulfillmentStatus = "inProgress"
  if (delivery.type === "Order") {
...
export const handleOrderFlow = async (delivery, type) => {
...
async function goBack(delivery) {
  ...
  if (delivery.type === "Order") {
    ...
    if(delivery.exchanged === true){
      ...
      if (docSnap.exists()) {
        ...
        handleOrderFlow(exchange, "outgoing")
      }
      await deleteDoc(doc(db, "flows", `E-${delivery.deliveryID}`));
      await deleteDoc(doc(db, "clients", `${delivery.client.clientID}/flows/E-${delivery.deliveryID}`));
    ...
    if (oldStatus === "success" ) {
      handleOrderFlow(delivery, "outgoing")
    }
...
export function CancelAlertInventory({ visible, handleVisibility, inventory }) {
  const handleSubmit = async (e) => {
    ...
    await deleteDoc(doc(db, "clients", `${inventory.client}/inventory/${inventory.inventoryID}`))
    handleVisibility(false)
  };
  if (!visible) return null;
  else {
...
export async function FlowToInventory(flow,oposite,fulfillmentStatus,purge){
  var auxflow = flow 
  ...
  if(purge){
    if(flow.type === "Return" ){
      ...
      if(flow.exchange){
          ...
          await deleteDoc(doc(db, "flows", `E-${flow.deliveryID}`));
          await deleteDoc(doc(db, "clients", `${flow.client.clientID}/flows/E-${flow.deliveryID}`));
...
export function CancelAlertFlow({ visible, handleVisibility, flow ,type}) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    FlowToInventory(flow,true,null,true) 
    handleVisibility(false)
  };
...
export function CancelAlertWarehouse({ visible, handleVisibility, warehouse }) {
  const {inventory} = UserAuth();
  const handleSubmit = async (e) => {
    e.preventDefault();
    for(let i = 0; i < inventory.length; i++){
      delete inventory[i]["inventory"][warehouse.id]
      await setDoc(doc(db, "clients", `${inventory[i].client}/inventory/${inventory[i].inventoryID}`), inventory[i])  
...
export function CancelAlertContacts({ visible, handleVisibility, contact }) {
  const handleSubmit = async (e) => {
    e.preventDefault();
    await deleteDoc(doc(db, "clients", `${contact.client}/contacts/${contact.contactID}`))
...
export function CancelAlertRecipient({ visible, handleVisibility, recipient }) {
  const handleSubmit = async (e) => {
    e.preventDefault();
    await deleteDoc(doc(db, "clients", `${recipient.client}/recipients/${recipient.recipientID}`))
  ...
  if (row.type === "Warehouse Managment") {
    return (
      <div key={row.fulfillmentStatus} style={{ display: "flex", justifyContent: "center" }} className="timer">
        ...
        {row.fulfillmentStatus !== "open" &&
        <img onClick={() => FlowToInventory(row,true,"open",null)} src={back} width={"22px"} style={{ display: "block", margin: "auto" }} />
        }
        {row.fulfillmentStatus === "open" &&
  ...
  if (status.startsWith("open")) {
    return (
      <div  key={value} style={{ display: "flex", justifyContent: "center" }} className="bike">
        ...
        <MessageAlert handleVisibility={setMessageAlertVisible} visible={messageAlertVisible} inProgressDelivery={inProgressDelivery} delivery={row} />
