...
export default function MessageForm({visible,handleVisibility}) {
  const {dropDownClients,dropDownContacts} = UserAuth();
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      inputFieldsGlobal[0].messageID = `${inputFieldsGlobal[0].client}-(${inputFieldsGlobal[0].to})-(${new Date().toLocaleString("sv", { timeZone: "Europe/Berlin"})})`
      if(checkEmptyValues(inputFieldsGlobal[0]) && inputFieldsGlobal[0].to.match('^[\+][(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$')){
        ...
        await setDoc(doc(db, "clients", `${inputFieldsGlobal[0].client}/messages/${inputFieldsGlobal[0].messageID}`), inputFieldsGlobal[0])
        setAlertVisible(false)
        clearMessageForm()
      }
      else{
    ...
    const setContactField = (event) =>{
      ...
      if(event){
        auxInputFields.contact = {name: event.value, contactID: event.contactID}
        auxInputFields.to = event.number
      }
      else{
        auxInputFields.contact = {name: "", contactID: ""}
        auxInputFields.to = ""
      }
